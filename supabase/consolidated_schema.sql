...
-- 11. Chat & Observability Telemetry (V1 MVP)
...
CREATE POLICY "Users can view their own chat telemetry" ON public.chat_telemetry FOR SELECT TO authenticated USING (auth.uid() = user_id);
CREATE POLICY "Users can insert their own chat telemetry" ON public.chat_telemetry FOR INSERT TO authenticated WITH CHECK (auth.uid() = user_id);
...
CREATE POLICY "Users can manage their own chat evaluations" ON public.chat_evaluations FOR ALL TO authenticated
  USING (EXISTS (SELECT 1 FROM public.chat_telemetry t WHERE t.id = telemetry_id AND t.user_id = auth.uid()))
  WITH CHECK (EXISTS (SELECT 1 FROM public.chat_telemetry t WHERE t.id = telemetry_id AND t.user_id = auth.uid()));
