      ...
      >
        ...
        <div style={{ marginBottom: 20 }}>
          ...
          <div style={{ display: 'flex', gap: 10 }}>
            {[
              ['ongoing', 'Ongoing', 'Persists across cycles'],
              ['cycle', 'This cycle only', 'Closes at cycle end'],
            ].map(([v, l, sub]) => (
              <button
                key={v}
                onClick={() => setScope(v)}
              ...
              >
                <div
                  style={{
                    fontSize: 13,
                    color: scope === v ? 'var(--color-text)' : 'var(--color-text-dim)',
                    marginBottom: 3,
                    fontFamily: "'DM Sans', sans-serif",
                  }}
                >
...
export function Rhythm({ userId, lunarData, loops = [] }) {
  ...
  const newMoonPrompts =
    lunarData?.phase?.key === 'new'
      ...
      {rhythms.length === 0 ? (
        ...
        >
          {rhythms.map((r, rIndex) => {
            ...
            else if (inst?.intentionType === 'phase')
              currentInt = inst.phaseIntentions?.[currentPhaseKey] || null;
