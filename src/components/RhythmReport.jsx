...
function describeEngagementPattern(observations) {
  ...
  const waxingAvg =
    waxingObs.length > 0 ? waxingObs.reduce((sum, p) => sum + obsMap[p], 0) / waxingObs.length : 0;
  const waningAvg =
    waningObs.length > 0 ? waningObs.reduce((sum, p) => sum + obsMap[p], 0) / waningObs.length : 0;
    ...
    >
      ...
      >
        ...
        {highestPhase && (
          <div style={{ fontSize: 11, color: 'var(--text-secondary)' }}>
            Peak:{' '}
            <span style={{ color: LEVEL_BADGE[highestPhase.engagement] }}>
      ...
      {(engagementPattern || intentionAlignment || quietestPhase) && (
        ...
        >
          {engagementPattern && (
            <div style={{ fontSize: 11, color: 'var(--text-secondary)' }}>{engagementPattern}</div>
          )}
          {intentionAlignment && (
            <div style={{ fontSize: 11, color: 'var(--text-secondary)' }}>
              Your practice matched intention in {intentionAlignment.matched} phase
              {intentionAlignment.matched !== 1 ? 's' : ''}
            </div>
          )}
          {quietestPhase && (
            <div style={{ fontSize: 11, color: 'var(--text-secondary)' }}>
      ...
      >
        ...
        {!loading && !reflection && observations.length > 0 && tried && (
          <div style={{ fontSize: 12, color: 'var(--text-disabled)' }}>Reflection unavailable.</div>
        )}
      </div>
    </div>
  );
